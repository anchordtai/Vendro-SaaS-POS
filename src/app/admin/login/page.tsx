...
import { Shield, Mail, Lock, Loader, AlertCircle, CheckCircle } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('vendro_session', JSON.stringify(session));
        ...
        <div className="max-w-md w-full mx-auto p-6">
          <div className="bg-white/10 backdrop-blur-lg rounded-xl p-8 border border-white/20 text-center">
            ...
            <p className="text-gray-300 mb-4">
              Redirecting to your Super Admin Dashboard...
            </p>
            <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4">
              <p className="text-green-400 text-sm">Welcome, Super Admin!</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-xl p-8 border border-white/20">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-300 text-sm">
              Access your Vendro POS administration panel
            </p>
          </div>
          ...
          <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-blue-400 text-sm font-medium mb-1">Custom Authentication</p>
                <p className="text-blue-300 text-xs">
                  This bypasses Supabase auth issues. Use the email you set up in the manual setup.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 text-center">
            <a 
              href="/admin/setup-manual" 
              className="text-purple-400 hover:text-purple-300 text-sm underline"
            >
              Need to set up a super admin?
            </a>
          </div>
        </div>
      </div>
